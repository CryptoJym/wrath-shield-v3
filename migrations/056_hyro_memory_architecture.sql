...
-- Migration: 056_hyro_memory_architecture.sql
-- Purpose: Create tables for student learning memory and cross-session continuity
...
SELECT
    ss.*,
    sm.data_json as student_profile,
    json_array_length(ss.items_administered) as items_count
FROM hyro_session_state ss
JOIN hyro_student_memory sm ON ss.student_id = sm.student_id
WHERE ss.status = 'active';

-- Learning trajectory summary
CREATE VIEW IF NOT EXISTS v_learning_trajectory AS
SELECT
    student_id,
    stat_name,
    date(started_at, 'unixepoch') as session_date,
    current_theta as ending_theta,
    standard_error as ending_se,
    json_array_length(items_administered) as items_count,
    total_time_seconds / 60.0 as duration_minutes
FROM hyro_session_state
WHERE status = 'completed'
ORDER BY student_id, started_at;

-- Record migration completion
INSERT INTO migrations (name) VALUES ('056_hyro_memory_architecture');
