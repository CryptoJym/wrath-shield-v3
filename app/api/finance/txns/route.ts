...
export async function GET(req: Request) {
  try {
    ...
    console.error('[finance/txns] Error:', e);
    return NextResponse.json({ error: e?.message || 'finance txns failed', txns: [] }, { status: 500 });
  }
}
...
export async function POST(req: Request) {
  try {
    ...
    console.error('[finance/txns POST] Error:', e);
    return NextResponse.json({ error: e?.message || 'finance txn update failed', ok: false }, { status: 500 });
  }
}
