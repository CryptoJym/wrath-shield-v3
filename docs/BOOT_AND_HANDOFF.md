...
- Vector DB (Qdrant, optional): http://localhost:6333

Quick Boot (all‑in‑one)
- From repo root: `bash scripts/start-all.sh`
- Optional: set `ALLOW_INMEMORY=1` to bypass the Qdrant preflight and run with in‑memory fallback.
...
- File: `.env.local`
  ...
  - `QDRANT_HOST=localhost`, `QDRANT_PORT=6333` (optional)
...
- MemoryWrapper may log a “mem0ai export mismatch” warning; guarded fallback to in‑memory or Qdrant. Does not block endpoints.
...
- Optional: docker‑compose for Qdrant + Streamlit for single‑command bring‑up
