...
async function runSimulation() {
    ...
    for (let i = 0; i < NUM_STUDENTS; i++) {
        ...
        for (let j = 0; j < ITEMS_PER_STUDENT; j++) {
            ...
            const itemDifficulty = Math.max(10, Math.min(90, targetDifficulty + (Math.random() - 0.5) * 5)); // Tighter selection

            // IRT Probability of correct response: P(theta) = 1 / (1 + e^(-k * (theta - b)))
            // theta = ability, b = difficulty
            const k = 0.15; // Sharper discrimination
            ...
            const learningRate = 0.5 / (1 + j * 0.1);
